...
module.exports = function (core, options) {
  ...
  core.on('ready', () => {
    const collection = core._collection.collection || core._collection
    collection.createIndexes([
      { key: { nextRunAt: -1, lastRunAt: -1, lastFinishedAt: -1 } },
      { key: { name: 1, nextRunAt: -1, lastRunAt: -1, lastFinishedAt: -1 } }
    ], err => {
      if (err) {
        console.warn('Timecloud-front indexes might not exist. Performance may decrease.')
      }
    })
    core._mdb.admin().serverInfo((err, serverInfo) => {
      if (err) {
        throw err
      }
      if (!semver.satisfies(serverInfo.version, '>=2.6.0')) {
        console.warn('Timecloud-front requires mongodb version >=2.6.0.')
      }
    })
  })
  ...
  const api = function (job, state, callback) {
    if (!core) {
      return callback('Timecloud-core instance is not ready')
    }
    async.parallel({
      ...
      (err, apiResponse) => {
        ...
        apiResponse.title = options.title || 'Timecloud'
        apiResponse.currentRequest = {
          title: options.title || 'Timecloud',
          job: job || '任务看板',
          state
        }
      ...
      collection
        ...
        .toArray((err, jobs) => {
          ...
          async.series(jobs.map(job => done => {
            const newJob = core.create(job.name, job.data).save(() => {
              done(null, newJob)
            })
          }), (err, results) => {
  ...
  const deleteJobs = (jobIds, callback) => {
    ...
    try {
      const collection = core._collection.collection || core._collection
      core.cancel({ _id: { $in: jobIds.map(jobId => collection.s.pkFactory(jobId)) } }, (err, deleted) => {
        if (err || !deleted) {
  ...
  const createJob = (jobName, jobSchedule, jobRepeatEvery, jobData, callback) => {
    ...
    try {
      ...
      const job = core.create(jobName, jobData)
